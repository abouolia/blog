...
<img src="https://miro.medium.com/v2/resize:fit:4800/format:webp/1*DqmRsX32NbQjFbmVFnPZSg.jpeg" />
...
The `ObserverProvider` that provider will flow the observer class instance to all sub-components. And that provider accepts `initialStat` property to inject an initial state to the `Provider` that would be passed to Observer.
